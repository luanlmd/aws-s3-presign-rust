    ...
    pub fn get_signed_url(options: &GetSignedUrlOptions, pre_signature: Option<Vec<u8>>) -> String
    {
        let signature_key = match pre_signature {
...
mod test {
    ...
    fn generate_signed_url()
    {
        ...
        let signed_url = get_signed_url(&options, None);
    ...
    fn generate_signed_url_with_presigned_key()
    {
        ...
        let signature_key = get_signature_key(&options);
        let signed_url = get_signed_url(&options, Some(signature_key));
        println!("Signed url {}", signed_url);
    }
}